...
contract UportRegistry{
  ...
  mapping (address => ListEntry[]) internal userAttributes;
  ...
  function set(bytes32 registrationIdentifier, address subject, bytes32 value, EidasLevel eidas) public validEntryData(msg.sender, subject, eidas){
      ...
      Set(registrationIdentifier, msg.sender, subject, eidas, accepted, now);
      registry[registrationIdentifier][msg.sender][subject] = RegistryEntry (value, eidas, accepted, true);
      userAttributes[subject].push(ListEntry(registrationIdentifier, msg.sender));
  }

  //get entry by id, issuer and subject
  function get(bytes32 registrationIdentifier, address issuer, address subject) public constant returns (bytes32 value, EidasLevel eidas, bool accepted, bool exists) {
      RegistryEntry storage entry = registry[registrationIdentifier][issuer][subject];
      return (entry.value, entry.eidas, entry.accepted, entry.exists);//should throw exception if not exists?
  }
  ...
  function getMyself(bytes32 registrationIdentifier, address issuer) public constant returns (bytes32 value, EidasLevel eidas, bool accepted, bool exists) {
      return get(registrationIdentifier, issuer, msg.sender);
  }

  //get my attributes
  function getMyAttributes() public constant returns (ListEntry[]) {
      return userAttributes[msg.sender];
  }
